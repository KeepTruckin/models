...
from object_detection.builders import lane_keypoint_predictor_builder
...
from object_detection.meta_architectures import ssd_kt_lane_meta_arch
...
def _build_ssd_kt_lane_model(ssd_kt_lane_config, is_training, add_summaries):
    num_classes = ssd_kt_lane_config.num_classes
    _check_feature_extractor_exists(ssd_kt_lane_config.feature_extractor.type)

    # Feature extractor
    feature_extractor = _build_ssd_feature_extractor(
        feature_extractor_config=ssd_kt_lane_config.feature_extractor,
        freeze_batchnorm=ssd_kt_lane_config.freeze_batchnorm,
        is_training=is_training)

    box_coder = box_coder_builder.build(ssd_kt_lane_config.box_coder)
    matcher = matcher_builder.build(ssd_kt_lane_config.matcher)
    region_similarity_calculator = sim_calc.build(
        ssd_kt_lane_config.similarity_calculator)
    encode_background_as_zeros = ssd_kt_lane_config.encode_background_as_zeros
    negative_class_weight = ssd_kt_lane_config.negative_class_weight
    anchor_generator = anchor_generator_builder.build(
        ssd_kt_lane_config.anchor_generator)
    if feature_extractor.is_keras_model:
        ssd_box_predictor = box_predictor_builder.build_keras(
            hyperparams_fn=hyperparams_builder.KerasLayerHyperparams,
            freeze_batchnorm=ssd_kt_lane_config.freeze_batchnorm,
            ...
            num_predictions_per_location_list=anchor_generator
                .num_anchors_per_location(),
            box_predictor_config=ssd_kt_lane_config.box_predictor,
            is_training=is_training,
            num_classes=num_classes,
            add_background_class=ssd_kt_lane_config.add_background_class)
    else:
        ssd_box_predictor = box_predictor_builder.build(
            hyperparams_builder.build, ssd_kt_lane_config.box_predictor, is_training,
            num_classes, ssd_kt_lane_config.add_background_class)

    # here add lane predictor head (mask rcnn keypoint predictor)
    keypoint_predictor = lane_keypoint_predictor_builder.build(hyperparams_builder.build,
                                                          ssd_kt_lane_config.keypoint_predictor,
                                                          is_training=is_training,
                                                          num_keypoints=ssd_kt_lane_config.keypoint_predictor.num_keypoints)

    image_resizer_fn = image_resizer_builder.build(ssd_kt_lane_config.image_resizer)
    non_max_suppression_fn, score_conversion_fn = post_processing_builder.build(
        ssd_kt_lane_config.post_processing)
    (classification_loss, localization_loss, keypoint_loss, classification_weight,
     localization_weight, keypoint_weight, hard_example_miner, random_example_sampler,
     expected_loss_weights_fn) = losses_builder.build_ssd_kt_lane_loss(ssd_kt_lane_config.loss)
    normalize_loss_by_num_matches = ssd_kt_lane_config.normalize_loss_by_num_matches
    normalize_loc_loss_by_codesize = ssd_kt_lane_config.normalize_loc_loss_by_codesize

    equalization_loss_config = ops.EqualizationLossConfig(
        weight=ssd_kt_lane_config.loss.equalization_loss.weight,
        exclude_prefixes=ssd_kt_lane_config.loss.equalization_loss.exclude_prefixes)
    ...
    ssd_kt_lane_meta_arch_fn = ssd_kt_lane_meta_arch.SSDKTLaneMetaArch
    kwargs = {}

    return ssd_kt_lane_meta_arch_fn(
        is_training=is_training,
        anchor_generator=anchor_generator,
        box_predictor=ssd_box_predictor,
        ...
        classification_loss_weight=classification_weight,
        localization_loss_weight=localization_weight,
        keypoint_loss_weight=keypoint_weight,
        normalize_loss_by_num_matches=normalize_loss_by_num_matches,
        hard_example_miner=hard_example_miner,
        target_assigner_instance=target_assigner_instance,
        add_summaries=add_summaries,
        normalize_loc_loss_by_codesize=normalize_loc_loss_by_codesize,
        freeze_batchnorm=ssd_kt_lane_config.freeze_batchnorm,
        inplace_batchnorm_update=ssd_kt_lane_config.inplace_batchnorm_update,
        add_background_class=ssd_kt_lane_config.add_background_class,
        explicit_background_class=ssd_kt_lane_config.explicit_background_class,
        random_example_sampler=random_example_sampler,
        expected_loss_weights_fn=expected_loss_weights_fn,
        use_confidences_as_targets=ssd_kt_lane_config.use_confidences_as_targets,
        implicit_example_weight=ssd_kt_lane_config.implicit_example_weight,
        equalization_loss_config=equalization_loss_config,
        return_raw_detections_during_predict=(
            ssd_kt_lane_config.return_raw_detections_during_predict),
        **kwargs)
...
META_ARCH_BUILDER_MAP = {
    ...
    'ssd_kt_lane': _build_ssd_kt_lane_model,
